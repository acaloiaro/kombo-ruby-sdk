...
module Kombo
  class GetAtsInterviewsPositiveResponseDataResultsInnerApplication < ApiModelBase
    ...
    def self.openapi_types
      {
        ...
        :'candidate' => :'GetAtsOffersPositiveResponseDataResultsInnerApplicationCandidate',
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `Kombo::GetAtsInterviewsPositiveResponseDataResultsInnerApplication` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `Kombo::GetAtsInterviewsPositiveResponseDataResultsInnerApplication`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def hash
      [id, remote_id, outcome, rejection_reason_name, candidate, job].hash
    end
