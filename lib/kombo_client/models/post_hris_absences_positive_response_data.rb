...
module Kombo
  class PostHrisAbsencesPositiveResponseData < ApiModelBase
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `Kombo::PostHrisAbsencesPositiveResponseData` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `Kombo::PostHrisAbsencesPositiveResponseData`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def hash
      [id, remote_id, employee_id, approver_id, start_date, end_date, start_half_day, end_half_day, start_time, end_time, amount, unit, status, employee_note, type_id, changed_at, remote_deleted_at, remote_data].hash
    end
