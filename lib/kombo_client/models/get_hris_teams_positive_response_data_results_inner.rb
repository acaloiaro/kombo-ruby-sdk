...
module Kombo
  class GetHrisTeamsPositiveResponseDataResultsInner < ApiModelBase
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `Kombo::GetHrisTeamsPositiveResponseDataResultsInner` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `Kombo::GetHrisTeamsPositiveResponseDataResultsInner`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def valid?
      ...
      type_validator = EnumAttributeValidator.new('String', ["DEPARTMENT", "TEAM", "COST_CENTER"])
    ...
    def type=(type)
      validator = EnumAttributeValidator.new('String', ["DEPARTMENT", "TEAM", "COST_CENTER"])
    ...
    def hash
      [id, remote_id, name, changed_at, remote_deleted_at, type, parent_id, remote_data].hash
    end
