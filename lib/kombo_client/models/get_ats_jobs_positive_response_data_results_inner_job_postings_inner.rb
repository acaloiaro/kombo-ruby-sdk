...
module Kombo
  class GetAtsJobsPositiveResponseDataResultsInnerJobPostingsInner < ApiModelBase
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `Kombo::GetAtsJobsPositiveResponseDataResultsInnerJobPostingsInner` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `Kombo::GetAtsJobsPositiveResponseDataResultsInnerJobPostingsInner`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def valid?
      ...
      status_validator = EnumAttributeValidator.new('String', ["ACTIVE", "INACTIVE", "DRAFT"])
      return false unless status_validator.valid?(@status)
      visibility_validator = EnumAttributeValidator.new('String', ["PUBLIC", "INTERNAL", "UNLISTED"])
    ...
    def status=(status)
      validator = EnumAttributeValidator.new('String', ["ACTIVE", "INACTIVE", "DRAFT"])
    ...
    def visibility=(visibility)
      validator = EnumAttributeValidator.new('String', ["PUBLIC", "INTERNAL", "UNLISTED"])
    ...
    def hash
      [id, remote_id, title, description_html, status, visibility, url, remote_data].hash
    end
