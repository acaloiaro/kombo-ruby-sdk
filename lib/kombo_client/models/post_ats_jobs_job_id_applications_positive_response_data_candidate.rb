...
module Kombo
  class PostAtsJobsJobIdApplicationsPositiveResponseDataCandidate < ApiModelBase
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `Kombo::PostAtsJobsJobIdApplicationsPositiveResponseDataCandidate` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `Kombo::PostAtsJobsJobIdApplicationsPositiveResponseDataCandidate`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def hash
      [id, remote_id, first_name, last_name, company, title, confidential, source, phone_numbers, email_addresses, social_media, location, custom_fields, integration_fields, remote_url, remote_created_at, remote_updated_at, remote_data, changed_at, remote_deleted_at, tags].hash
    end
